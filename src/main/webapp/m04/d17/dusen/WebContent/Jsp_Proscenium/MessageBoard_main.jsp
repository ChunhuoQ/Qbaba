...
<html>
  <head>
	...
	<!-- <link rel="stylesheet" type="text/css" href="ccs/style_admin.css"> -->
	<style type="text/css">
	<!--
	.newMessage {
		font-size: 12px;
	        ...
	        <table width="750" border="0" cellspacing="0" cellpadding="0" style="font-size:12px;">
			  ...
			  <c:if test="${not empty listm}">
			  	<c:forEach items="${listm }" var="i">
	           	  <tr>
		            <td class="lb"><div align="center">${i.rcount }</div></td>
		            <td height="25" class="lb">
		            <span class="gray">&nbsp;<img src="Img_Background/1/jd-to.gif" width="13" height="13" border="0" />&nbsp;
		            <a href="<%=path %>/MServlet1?mid=${i.mid }" target="_parent">${i.mtitle }</a></span></td>
		            <td class="lb"><div align="center"><span class="gray">${i.mname }</span></div></td>
		            <td class="lrb"><span class="gray">&nbsp;${i.mdate }</span></td>
		          </tr>
			  	</c:forEach>
			  </c:if>
	        </table>
