...
/*select reader.`rname` as 读者,book.`bname` as 书名,penalty.`pdate` as 罚款日期
 from reader,book,penalty where reader.`rid`=1; and book.`bid`=1;penalty.`bid` 
*/
...
SELECT bw.rid AS '读者编号',rd.rname AS '读者姓名',
bk.bname AS '图书名称',bw.`lenddate` AS'借书日期',bw.`willdate` AS'应还日期'
FROM borrow bw 
INNER JOIN book bk ON bw.`nif`=bk.`bid` 
INNER JOIN reader rd ON bw.`rid`=rd.`rid`
 WHERE returndate IS NULL AND  bw.`rid` IN
